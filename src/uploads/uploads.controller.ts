...
export class UploadsController {
  ...
  complete(@Req() req, @Param("uploadId") uploadId: string, @Body() parts: Array<{partNumber: number, etag: string}>) {
    const userId = req.user.id;
  ...
  signPart(@Req() req, @Param("uploadId") uploadId: string, @Param("partNumber") partNumber: number) {
    const userId = req.user.id;
